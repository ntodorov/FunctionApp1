...
{
    public class SectionTable : IPDFTable
    {
        ...
        public void AddTitle(string title, string subTitle, int colKount)
        {
            this.title = title;
            Aspose.Pdf.Cell xcelll;
            ...
            TextState txtState2 = new TextState();
            txtState2.Font = FontRepository.FindFont(mainFont);
            txtState2.FontSize = 11;


            Aspose.Pdf.MarginInfo titleMargin = new Aspose.Pdf.MarginInfo();
            titleMargin.Top = 3f;
            titleMargin.Left = 0f;
            titleMargin.Right = 0f;
            titleMargin.Bottom = 1f;


            var row = table.Rows.Add();
            titleCell = row.Cells.Add(title);
            titleCell.Alignment = HorizontalAlignment.Left; titleCell.VerticalAlignment = VerticalAlignment.Top;
            titleCell.DefaultCellTextState = txtState1;
            titleCell.Margin = titleMargin;


            xcelll = row.Cells.Add(subTitle); xcelll.ColSpan = colKount - 1;
            xcelll.Alignment = HorizontalAlignment.Left; xcelll.VerticalAlignment = VerticalAlignment.Top;
            xcelll.DefaultCellTextState = txtState2;
            xcelll.Margin = titleMargin;
        ...
        public Row AddRow(string Caption, string Value, string Value2, string Value3)
        {
            var val = Value == null ? string.Empty : Value;
            var val2 = Value2 == null ? string.Empty : Value2;
            var val3 = Value3 == null ? string.Empty : Value3;

            var row = table.Rows.Add();
            var xcell = row.Cells.Add(Caption);
            xcell = row.Cells.Add(val);
            xcell = row.Cells.Add(val2);
            xcell = row.Cells.Add(val3);
        ...
        public Row AddRow(string Caption, Table PDFTable)
        {
            var row = AddRow(Caption, "");
        ...
        public Row AddPDFTable(Table table)
        {
            var row = AddHeadingRow("");
            var xcell = row.Cells[0];

            xcell.Paragraphs.Clear();
            xcell.Paragraphs.Add(table);

            return row;
        }

        public Row AddTextRow(string Text)
        {
            var row = table.Rows.Add();
            var xcell = row.Cells.Add(Text);
            xcell.ColSpan = DefaultCollSpan;
